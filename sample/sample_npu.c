...
******************************************************************************/
void SAMPLE_SVP_NPU_Dcnn(SC_CHAR *md)
{
...
void SAMPLE_SVP_NPU_Yolov3_file(int model_type,SC_CHAR*npu_model,SC_CHAR*src_image){
	...
	SC_CHAR *pcSrcFile = src_image;
	SC_CHAR *pcModelName=npu_model;
	printf("model_path=%s,pcSrcFile=%s,model_type=%d\n",pcModelName,pcSrcFile,model_type);
	SC_S32 s32Ret = SC_FAILURE;
...
//    yolov3_image.astInputImg[0].stImages[0].enType = SVP_IMG_YUV420P;
	...
	//读取图片数据,转化为yuv数据传入模型
	#if 0
	cv::Mat rgb_img = cv::imread(pcSrcFile);
	if(!rgb_img.empty()){
		SAMPLE_SVP_TRACE_ERR("pcSrcFile ; %s is not exist!\n",pcSrcFile);
	}
	cv::Mat resize_img,yuv_img;
	cv::resize(rgb_img, resize_img, cv::Size(model_h,model_w), cv::INTER_LINEAR);
	cv::cvtColor(resize_img, resize_img, cv::COLOR_BGR2RGB);
	cv::cvtColor(resize_img, yuv_img, cv::COLOR_RGB2YUV);
	char *p_data = (char*)(yuv_img.data);
	#endif
...
//    s32Ret = SAMPLE_COMM_SVP_NPU_FillSrcData(pcSrcFile, &s_stYolov3NpuParam, &yolov3_image, SVP_IMG_YUV420P);
	...
	s32Ret = SAMPLE_COMM_SVP_NPU_FillSrcData_file(pcSrcFile, &s_stYolov3NpuParam, &yolov3_image, model_types);
	SAMPLE_SVP_CHECK_EXPR_GOTO(SC_SUCCESS != s32Ret, YOLOV3_FAIL_0, SAMPLE_SVP_ERR_LEVEL_ERROR,
	                           "Error,SAMPLE_SVP_NPU_FillSrcData failed!\n");
	/*NPU process*/
	...
	printf("A200 SDK yolov3_sample cost time = %f s\n",duration);
...
//	Mat mat;
//	SC_CHAR *pcSrcFile = image_name;
	SC_CHAR *pcModelName=npu_model;
	printf("model_path=%s,model_type=%d\n",pcModelName,model_type);
    SC_S32 s32Ret = SC_FAILURE;
