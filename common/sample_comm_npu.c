...
SC_S32  SAMPLE_COMM_SVP_NPU_FillSrcData_file(char *image_path, SAMPLE_SVP_NPU_PARAM_S *pstNpuParam,
	...
	if(enImageType == SVP_IMG_YUV420P)
	{
		...
		else
		{
			...
			pstImg->astInputImg[0].stImages[0].au64PhyAddr[2] = y_addr_phy + u16Stride * u32Height + u32Width * u16Stride / 4;
	...
	else if(enImageType == SVP_IMG_RGB)
	{
		...
		fseek (fp, 0, SEEK_END);
		SC_U32 image_size = ftell (fp);
		SC_U32 input_size = u32Width * u32Height * 3;
		printf("image_size=%d,input_size=%d\n",image_size,input_size);
		...
		SC_CHAR *buf = (SC_CHAR *)malloc(image_size);
		if(buf == NULL)
		{
			SAMPLE_SVP_TRACE_ERR("malloc error\n");
			fclose(fp);
...
//	    npu_conf.stModelBuf.u64VirAddr = (SC_U64)bin_buf;
//	    npu_conf.stModelBuf.u64Size = len;
...
SC_S32  SAMPLE_COMM_SVP_NPU_FillSrcData(char *p_data, SAMPLE_SVP_NPU_PARAM_S *pstNpuParam,
                                        NPU_IMAGE_S *pstImg, SVP_IMAGE_TYPE_E enImageType)
{
	SC_U32 u32Width = pstImg->astInputImg[0].stImages[0].u32Width;
	SC_U32 u32Height = pstImg->astInputImg[0].stImages[0].u32Height;
	...
	if(enImageType == SVP_IMG_YUV420P)
	{
		...
		for(int i = 0; i < u32Height; i++)
		{
			memcpy((char *)(y_addr + i * u16Stride), p_data + i * u32Width, u32Width);
		}
		
		char *u = (char *)(y_addr + u16Stride * u32Height);
		SC_CHAR *src = p_data + u32Height * u32Width;
...
//	    npu_conf.stModelBuf.u64Size = len;
        ...
        SC_CHAR *pBuf = (SC_CHAR *)((SC_U32)pstNpuParam->PictMem.u64VirAddr);
        SC_CHAR *pchR = p_data;
        SC_CHAR *pchG = p_data + u32Height * u32Width;
        SC_CHAR *pchB = p_data + 2 * u32Height * u32Width;
...
//        free(buf);
		...
		unsigned short u16Stride = ALIGNED_256B(u32Width);
		SC_CHAR *pBuf = (SC_CHAR *)((SC_U32)pstNpuParam->PictMem.u64VirAddr);
		memcpy(pBuf,p_data,u16Stride*u32Height*3);
