...
extern "C" int yolovx_postpross_and_getresult(void *pTensor, int tensorNum, uintptr_t virtAddr, void **pResult,
    int *ResultNum, int yolovx, int w, int h,float *anchor,float confidence_th,float nms_th);
...
YoloDetection::YoloDetection(output_tensor *pTensorInfo, int tensorNum, int *inputTensorSize, float *anchorSize,
    float confThres, float iouThres)
{
    ...
    this->anchorSize = (float *)malloc(this->outputTensorNum * ANCHOR_SPECIES * 2 * sizeof(float));
    memcpy((void *)this->anchorSize, anchorSize, this->outputTensorNum * ANCHOR_SPECIES * 2 * sizeof(float));
	#if Debug
    for(int i=0;i<18;i++){
    	printf("anchor[i]=%f\n",i,this->anchorSize[i]);
    }
	...
	#if Debug
    printf("this->pTensor[0].u32OriChannels  = %d \n",this->pTensor[0].u32OriChannels);
    printf("**********totalClass***************** = %d\n",totalClass);
	#endif
...
int YoloDetection::yoloBboxDecoding(short *yolovBuffer, int tensorIndex, detection *detsObj)
{
    ...
    printf("feature_h=%d,feature_w=%d\n",feature_h,feature_w);
...
int YoloDetection::yoloPostProcess(short *buff, vector<detection_output> &vOutput)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (prob > this->confThres)
        {
            ...
            obj.classId = obj_id;
	        #if Debug
            printf("x1=%f,y1=%f,x2=%f,y2=%f,score=%f,class_id=%d\n",
                   obj.x,obj.y,obj.w+obj.x,obj.y+obj.h,obj.confidence,obj.classId);
			#endif
...
int yolovx_postpross_and_getresult(void *pTensor, int tensorNum, uintptr_t virtAddr, void **pResult, int *ResultNum,
    int yolovx, int w, int h,float *anchor,float confidence_th,float nms_th)
{
    g_yolovx = yolovx;
    /*
    int ret = yolovx_init();
	...
	*/
    int model_step4 = (w/32)*4;
    int model_step2 = (w/32)*2;
	int model_step = (w/32);
	MAX_DETCTION_NUMBER=(model_step4 * model_step4 * 3 + model_step2 * model_step2 * 3 + model_step * model_step4 * 3);
    ...
    YoloDetection yolo = YoloDetection(ptensor, tensorNum, inputTensorSize, anchor, confidence_th,
            nms_th);

    if(!strcmp("float", ptensor[0].achType))
    {
    	printf(" start to float !\n");
    ...
    {
    	printf("detectionOutputVector is empty!\n");
        *pResult = NULL;
        *ResultNum = 0;
    }
    else
